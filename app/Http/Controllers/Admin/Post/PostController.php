...
//                $url = asset('assets/images/posts/' . $data->images);
            ...
            ->addColumn('action', function ($data) {
                ...
                $button = '<a   id="' . $data->id . '" url="'.$data->url.'"title_ar="' . $data->getTranslation('title', 'ar') . '" title_en="' . $data->getTranslation('title', 'en') . '"  image="'.$data->Images().'"  class="edit_post"><span><i style="color: #66afe9" class="fas fa-edit"></i></span></a>';
                $button .= '&nbsp;&nbsp;&nbsp;&nbsp;';
    ...
    public function store(Request $request){
        try {
            if (isset($request->post_id)){
                ...
                $post->title = ['ar' => $request->title_ar, 'en' => $request->title_en];
